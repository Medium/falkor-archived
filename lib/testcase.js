...
TestCase.prototype.toNodeUnitFn = function () {
  ...
  var fn = function (test) {
    if (testCase._prev) {
      testCase._prev._noteUnitFn(test)
    }
    testCase.setAsserter(test)
    testCase.run()
  }
  testCase._noteUnitFn = fn
    ...
    (function (k) {
      ...
      if (k.charAt(0) != '_' && typeof testCase[k] == 'function') {
        fn[k] = function () {
          ...
          // If 'fetchAgain' is called, we want to return a new
          // wrapper for the new TestCase instance.
          if (k == 'fetchAgain') {
            return testCase._next.toNodeUnitFn()
          } else {
            return fn
          }
        }
...
/**
 * Fetch a new URL. It inherits the previous template, but it can
 * have its own properties and expectations.
 */
TestCase.prototype.fetchAgain = function (url) {
  var testCase = new TestCase(url)
  testCase._copy(this._template)
  testCase._setTemplate(this._template)
  this._next = testCase
  testCase._prev = this
}
...
TestCase.prototype._finalize = function (res) {
  ...
  // If there are more testcases chained after this one, we don't
  // want to call 'done()' yet.
  if (!this._next) {
    this._asserter.done()
  }
}
...
TestCase.prototype._setTemplate = function (template) {
