...
function TestCase(url) {
  ...
  /** Function called when the test is completed. */
...
TestCase.prototype.toNodeUnitFn = function () {
  ...
  // finalize the test.
  var fn = function (test, callback) {
    testCase.setAsserter(test)
    testCase.run(callback)
  }
 ...
 * @param {function(Object)=} opt_callback Optional callback to execute when the test has completed,
 *     if specified the callback is responsible for calling `done` on the test object that is passed
 *     as the argument.
 */
TestCase.prototype.run = function (opt_callback) {
  if (!this._asserter) throw Error('No asserter object has been configured')

  this._doneCallback = opt_callback
...
TestCase.prototype._finalize = function (res) {
  ...
  var waitCount = this._chained.length

  // Run all the chained test cases.
  for (var i = 0; i < this._chained.length; i++) {
    var rv = this._chained[i].call(this, res)

    if (rv instanceof TestCase) {
      rv.setAsserter(this._asserter)
      rv.run(onChainedFinish.bind(this))

    } else if (typeof rv == 'function') {
      rv(this._asserter, onChainedFinish.bind(this))
  ...
  maybeFinish.call(this)

  function maybeFinish() {
    if (waitCount == 0) {
      if (this._doneCallback) this._doneCallback(this._asserter)
      else this._asserter.done()
    }
  }

  function onChainedFinish() {
    waitCount--
    maybeFinish.call(this)
  }
}
