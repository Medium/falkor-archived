 ...
 * are referenced by validateJsonById().
 *
 * Unlike validateJson(), every schema processed by addJsonSchema
 * must have an id.
 *
 * JSON schema that are added using this method are available to be
 * referenced by other JSON schema objects using $ref, with the id
 * of the schema as the value for the $ref.
 *
...
Options.prototype.addJsonSchema = function (schemaPath) {
  ...
  var schemaFile, schemata, schema
  ...
  this.evaluate(function (test, res) {
    ...
    if (schemata instanceof Array) {
      for (var i = 0; i < schemata.length; i++) {
        this._doAddJsonSchema(test, res, schemata[i]);
      }
    } else {
      this._doAddJsonSchema(test, res, schemata);
    }
  })
...
/**
 * Private method that adds a JSON schema file to our internal map
 ...
 * @private
 */
Options.prototype._doAddJsonSchema = function (test, res, schema) {
  if (!schema.id) {
    test.fail('JSON schema must have an id before calling addJsonSchema: ' + schema)
    return
  }
  if (this._fixSchema(test, res, schema)) this._jsonSchema[schema.id] = schema
}
 ...
 * @private
 */
Options.prototype._fixSchema = function (test, res, schema) {
  for (var key in schema) {
    if (key == 'pattern') {
      ...
      if (!this._fixSchema(test, res, schema[key])) return false
    }
  }
...
/**
 * Validates the response against a JSON Schema in the provided file.
 * @param {string} schemaPath Path to the schema file.
 * @return {Options} The instance.
 */
Options.prototype.validateJsonById = function (schemaId) {
  this.evaluate(function (test, res) {
    var schema = this._jsonSchema[schemaId]

    if (!schema) {
      test.fail('A JSON schema with id ' + schemaId + ' could not be found.')
      return
    }
    this._doValidateJson(test, res, schema)
  })
...
Options.prototype.validateJson = function (schemaPath) {
  ...
  this.evaluate(function (test, res) {
    ...
    if (this._fixSchema(test, res, schema)) this._doValidateJson(test, res, schema)
  })
...
/**
 * Private method that does the guts of testing a JSON file against a
 * given schema.
 * @param {Object} test An object which exposes the nodeunit test interface.
 * @param {Object} res The response object.
 * @param {Object} schema The json schema object to test against.
 * @private
 */
Options.prototype._doValidateJson = function (test, res, schema) {
  if (!res.body) {
