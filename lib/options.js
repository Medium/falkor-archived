...
function Options() {
  ...
  /** Optional setup function for individual test case. */
  this._setupFn = null
  ...
  /** A global base URL from which other URLs are resolved. */
  this._baseUrl = ''

  /** A global path from which schema paths are resolved. */
  this._rootSchemaPath = ''

  /** @private {number} The default timeout in milliseconds */
  this._timeoutMs = -1

  // NOTE: When adding options make sure to add a corresponding step in mixin().
}
module.exports = Options


/**
 * Adds a function to be called just before the test case for preparing/setting up things.
 * Note that if the test file has 'beforeEach' function defined, it will overwrite
 * individual setup functions.
 * @param {Function} setupFn
 * @return {Options} The instance
 */
Options.prototype.usingSetup = function (setupFn) {
 ...
 * @return {Options} The instance.
 */
Options.prototype.withCookie = function (name, value) {
 ...
 * @return {Options} The instance.
 */
Options.prototype.withBaseUrl = function (baseUrl) {
 ...
 * @return {Options} The instance.
 */
Options.prototype.withRootSchemaPath = function (rootSchemaPath) {
  this._rootSchemaPath = rootSchemaPath
  return this
}
 ...
 * @return {Options} The instance.
 */
Options.prototype.withTimeoutMs = function (timeout) {
...
/**
 * Adds an evaluator function that will be called with the JSON object parsed from the body of
 * the response, and the asserter which they can use to perform assertions against the JSON
 * content.
 *
 * It asserts a failure if the JSON parsing does not succeed.
 *
 * @param {function (this:Options, Object, JsonObject)} fn
 * @return {Options} The instance.
 */
Options.prototype.evaluateWithJsonBody = function (fn) {
  this.evaluate(function (test, res) {
    var json = this._getJsonFromResponse(test, res)
    if (json == null) {
      return
    }
    fn(test, json)
...
Options.prototype.addJsonSchema = function (schemaPath) {
  if (this.getRootSchemaPath()) {
    schemaPath = path.resolve(this.getRootSchemaPath(), schemaPath)
  }
...
/**
 * Copies the settings from another Options object to this one in an additive
 * fashion.
 * @param {Options} source
 * @return {Options}
 */
Options.prototype.mixin = function (source) {
  this._mixin(source._headers, this._headers)
  ...
  if (source._baseUrl) this._baseUrl = source._baseUrl
  if (source._rootSchemaPath) this._rootSchemaPath = source._rootSchemaPath
  if (source._timeoutMs > 0) this._timeoutMs = source._timeoutMs

  this._evaluators = this._evaluators.concat(source._evaluators)
...
/**
 * Extracts JSON from the response, failing the test if there is a parse error.
 */
Options.prototype._getJsonFromResponse = function (test, res) {
 ...
 * @param {Object} dest
 */
Options.prototype._mixin = function (source, dest) {
 ...
 * @protected
 */
Options.prototype.getParsedUrl = function () {
  var baseUrl = this._baseUrl || config.baseUrl
  return urlLib.parse(baseUrl ? urlLib.resolve(baseUrl, this._url) : this._url)
}
 ...
 * @protected
 */
Options.prototype.getRootSchemaPath = function () {
  return this._rootSchemaPath || config.rootSchemaPath
}
